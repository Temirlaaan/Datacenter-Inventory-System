"""Web admin OIDC authorization-code flow (extracted from ``app.web.router``).

Three routes on their own ``APIRouter`` (mounted by the aggregator in
``app.web.router``):

- ``GET /web/login`` — 302 to Keycloak's authorization endpoint with ``state``
  + ``nonce`` stored as short-lived cookies the callback verifies.
- ``GET /web/oidc/callback`` — verifies ``state``, exchanges the auth code for
  tokens, sets the encrypted ``dcinv_admin_session`` cookie, 302s to ``next``
  (validated as a local path) or ``/web/``.
- ``GET /web/logout`` — clears the session cookie + 302s to Keycloak end-session.

Cookie encode/decode + ``require_web_admin`` live in ``app.web.auth``; this
module only drives the redirect handshake.
"""
...
def _safe_next_path(next_path: str) -> str:
    """Constrain the post-login redirect target to a local path.

    ``next`` is attacker-controllable (``/web/login?next=...``) and is later
    used as a ``RedirectResponse`` target after the victim authenticates —
    an unconstrained value is an open redirect (``next=https://evil.com`` or
    the protocol-relative ``next=//evil.com``). We only ever want to bounce
    back to a page inside this app, so reject anything that isn't a single-
    leading-slash relative path and fall back to the dashboard.
    """
...
async def login(request: Request, next: str = "/web/") -> RedirectResponse:
    ...
    for cookie_name, cookie_value in (
        ...
        (_OIDC_NEXT_COOKIE, _safe_next_path(next)),
    ):
        response.set_cookie(
            cookie_name,
            cookie_value,
...
async def oidc_callback(
    ...
    next_path = _safe_next_path(request.cookies.get(_OIDC_NEXT_COOKIE, "/web/"))
    ...
    try:
        ...
        # A token without a (valid UUID) `sub` is malformed — reject as a 400
        # rather than letting `UUID(None)` raise an unhandled 500. Keycloak
        # always issues a UUID sub, so this only fires on a misconfigured IdP.
        logger.warning("web_oidc_callback_missing_sub", has_sub=sub_raw is not None)
        return HTMLResponse(
            "OIDC callback rejected: id_token has no valid subject.",
            status_code=status.HTTP_400_BAD_REQUEST,
        )
    ...
    user = build_session_cookie_payload(sub=sub, email=email, roles=roles)
