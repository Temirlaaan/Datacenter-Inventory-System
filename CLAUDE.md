...
Sprint 6 (Shift Sessions) closed 2026-05-30. The `backend/` directory contains a runnable FastAPI service with auth, async DB + Alembic, NetBox read+write client, `/health`, and a docker-compose stack. Business surface so far: Sprint 2's QR registry (`POST /api/v1/admin/batches/`, `GET /api/v1/admin/batches/{id}`); Sprint 3's device read + update (`GET`/`PATCH /api/v1/devices/{id}`) on the three-record-write apparatus, plus NetBox static lookups behind a 5-minute cache (`GET /api/v1/meta/{sites,racks,statuses}`) and the server-driven device-edit form config (`GET /api/v1/meta/device-form`); Sprint 4's QR lifecycle — `POST /api/v1/qr/{id}/bind` (role `dcinv-mobile-user`) and `POST /api/v1/qr/{id}/retire` (role `dcinv-admin`) with atomic free→bound and bound→retired transitions plus explicit three-branch compensation (clear/restore conditional and idempotent), and the combined `GET /api/v1/qr/{id}` returning QR + bound-device in one call (response shape changed from Sprint 2's flat shape — see work-log); Sprint 5's device write completion — `POST /api/v1/devices/` (role `dcinv-mobile-user`) for device create with structured 422 translation of NetBox validation errors via the new `NetBoxValidationError` subclass, `POST /api/v1/devices/{id}/comments` (role `dcinv-mobile-user`) appending a NetBox journal entry without a device PATCH, and `POST /api/v1/devices/{id}/decommission` (role `dcinv-admin`) with QR-first ordering and three-branch re-bind compensation on device-PATCH failure. Task 1 of Sprint 5 also generalised the apparatus: `NetBoxWriteService.post_with_attribution` is the create-path peer of `patch_with_attribution`, and `QRLifecycleService.retire` now returns `tuple[QR, dict | None]` so decommission can capture the post-retire device version for the deterministic OCC token. Sprint 6's shift sessions — three mobile-driven endpoints (`POST /api/v1/sessions/start` body `{tablet_id}`, `POST /api/v1/sessions/end` body `{end_reason: "manual" | "inactivity_timeout"}` — `admin_force_close` is reserved for a Sprint 7+ admin endpoint, `GET /api/v1/sessions/active` returning `{"session": null}` when none) backed by the new `shift_sessions` table (Postgres-enforced "≤1 active per user" via partial unique index + `shift_end_consistency` CHECK pairing `shift_end_at` with `end_reason`). The big cross-cutting change is **the source of `audit_log.session_id` flipped from the JWT `sid` claim (ephemeral; rotates during a shift) to `shift_sessions.id` (one per engineer-shift)** — semantic-only change, no schema migration, pre-Sprint-6 rows still hold their JWT-sid values. Six write endpoints (`POST /qr/{id}/{bind,retire}` and `POST/PATCH /devices/{,id,id/comments,id/decommission}`) are now gated by the dep-layer `require_role_with_active_shift(role)` and return structured 409 `NO_ACTIVE_SHIFT` when the JWT-identified user has no open shift. Keycloak refresh-token revocation is mobile's job after `/sessions/end` (decision J — backend stays out of Keycloak admin). The 10-minute inactivity auto-end is mobile-owned (calls `/sessions/end` with `end_reason="inactivity_timeout"`); a 12-hour-orphan backend fallback job for crashed tablets is Sprint 7+. `POST /api/v1/admin/batches/` is deliberately NOT gated yet (admin sessions surface lands later). Still to come — auto-end stale-sessions job, admin sessions list + force-close (`admin_force_close` reason), `GET /api/v1/admin/audit` query, PDF labels, web admin, and `GET /api/v1/meta/device-create-form`.
